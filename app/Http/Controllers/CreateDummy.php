...
class CreateDummy extends Controller
{
    ...
    {
        $cred = [
            ['username' => 'pyhsician', 'role' => 'pyhsician', 'password' => bcrypt('pyhsician')],
            ['username' => 'nurse', 'role' => 'nurse', 'password' => bcrypt('nurse')],
            ['username' => 'admin', 'role' => 'admin', 'password' => bcrypt('admin')],
            ['username' => 'technician', 'role' => 'technician', 'password' => bcrypt('technician')],
            ['username' => 'staff', 'role' => 'staff', 'password' => bcrypt('staff')],
        ];

        foreach ($cred as $key => $value) {
            if ($value['username'] == 'pyhsician')
                $login = Physician::all()->random(1)->first();
            else if ($value['username'] == 'nurse')
                $login = Nurse::all()->random(1)->first();
            else if ($value['username'] == 'admin')
                $login = Staff::all()->random(1)->first();
            else if ($value['username'] == 'staff')
                $login = Staff::all()->random(1)->first();
            else if ($value['username'] == 'technician')
                $login = Technician::all()->random(1)->first();
            ...
            if ($value['username'] == 'pyhsician') {
                $user->physician()->save($login);
            } else if ($value['username'] == 'nurse') {
                $user->nurse()->save($login);
            } else if ($value['username'] == 'admin') {
                $user->staff()->save($login);
            } else if ($value['username'] == 'staff') {
                $user->staff()->save($login);
            } else if ($value['username'] == 'technician') {
                $user->technician()->save($login);
            }
        }
    }
